...
#define BIT_DELAY_TIME 1000  // AUMENTADO: 500→1000 para mayor robustez
#define BYTE_DELAY_TIME 5000 // Pausa entre bytes para sincronización
...
static void send_byte(u8 byte)
{
    ...
    udelay(BIT_DELAY_TIME * 3); // Pausa robusta para sincronización (AUMENTADA)
...
static int receive_byte(void)
{
    ...
    timeout = 500000000; // AUMENTADO: timeout más largo para robustez
    ...
    // Start bit: 0-1000 μs
    // Centro de Bit 0: ~1500 μs desde inicio del start bit
...
static ssize_t dev_write(struct file *file, const char __user *buf, size_t len, loff_t *offset)
{
    ...
    // Preparar los 5 bytes a enviar
    bytes_to_send[0] = (u8)cmd;
    bytes_to_send[1] = (u8)(x >> 8);
    bytes_to_send[2] = (u8)(x & 0xFF);
    bytes_to_send[3] = (u8)(y >> 8);
    bytes_to_send[4] = (u8)(y & 0xFF);

    // Enviar cada byte con ACK individual y pausa
    for (i = 0; i < 5; i++)
    {
        pr_info("dev_write: enviando byte %d de 5: 0x%02X\n", i + 1, bytes_to_send[i]);

        send_byte(bytes_to_send[i]);

        // CAMBIO CRÍTICO: Esperar ACK tras cada byte individual
        // Esto permite que el ESP confirme que recibió cada byte correctamente
        ack = receive_byte();
        if (ack < 0)
        {
            pr_alert("dev_write: timeout recibiendo ACK para byte %d\n", i);
            return -ETIMEDOUT;
        }

        if ((u8)ack != 'A')
        {
            pr_alert("dev_write: ACK invalido en byte %d (recibido: 0x%02X)\n", i, (u8)ack);
            return -EIO;
        }

        pr_info("dev_write: ACK recibido para byte %d\n", i + 1);

        // Pausa entre bytes para sincronización
        udelay(BYTE_DELAY_TIME);
    }

    pr_info("dev_write: todos los bytes enviados y confirmados correctamente\n");
    return len;
}
